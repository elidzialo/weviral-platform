    ...
    >
      {step === 'success' ? (
        <div className="py-8 text-center">
          ...
          <h3 className="text-lg font-semibold text-[#0B0B0C] mb-1">Proof Submitted!</h3>
        ...
        <div className="space-y-5">
          {/* Campaign info */}
          <div className="bg-[#F6F6F3] rounded-xl px-4 py-3 border border-[#ECECE8]">
            <p className="text-xs text-[#8C8C88] font-medium uppercase tracking-wider mb-1">Campaign</p>
            <p className="text-sm font-semibold text-[#0B0B0C]">{campaign?.title ?? 'Campaign'}</p>
            <div className="flex gap-3 mt-1.5 text-xs text-[#8C8C88]">
          ...
          {/* Screenshot upload */}
          <div>
            <label className="block text-sm font-medium text-[#0B0B0C] mb-1.5">
              Screenshot <span className="text-red-500">*</span>
              <span className="text-[#8C8C88] font-normal ml-1">(JPG, PNG, WebP — max 50 MB)</span>
            </label>
            <input
            ...
            />
            {screenshotPreview ? (
              <div className="relative rounded-xl overflow-hidden border border-[#ECECE8]">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={screenshotPreview}
                  alt="Screenshot preview"
                  className="w-full h-48 object-contain bg-[#F6F6F3]"
                />
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="absolute top-2 right-2 p-1.5 bg-white border border-[#ECECE8] rounded-full shadow text-[#8C8C88] hover:text-[#0B0B0C]"
              ...
              >
                ...
                <p className="text-xs text-[#8C8C88] mt-1">Must show WhatsApp Status with view count</p>
              </button>
            )}
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-[#0B0B0C] mb-1.5">
              Video <span className="text-[#8C8C88] font-normal">(optional — MP4, MOV, WebM — max 50 MB)</span>
            </label>
            <input
              ref={videoInputRef}
              type="file"
            ...
            />
            {videoFile ? (
              <div className="flex items-center gap-3 bg-[#F6F6F3] border border-[#ECECE8] rounded-xl px-4 py-3">
                <svg className="w-5 h-5 text-[#8C8C88] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15 10l4.553-2.276A1 1 0 0121 8.723v6.554a1 1 0 01-1.447.894L15 14M3 8a2 2 0 012-2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V8z" />
                </svg>
                <span className="text-sm text-[#0B0B0C] flex-1 truncate">{videoFile.name}</span>
                <button
                  type="button"
                  onClick={() => {
                    setVideoFile(null);
                  ...
                  className="text-[#8C8C88] hover:text-[#8C8C88]"
          ...
          {(step === 'uploading' || step === 'verifying') && (
            <div className="bg-[rgba(110,91,255,.06)] border border-[#6E5BFF]/20 rounded-xl px-4 py-3 flex items-center gap-3">
              <svg className="animate-spin w-5 h-5 text-[#6E5BFF] flex-shrink-0" fill="none" viewBox="0 0 24 24">
...
const ApplicationRow: React.FC<ApplicationRowProps> = ({ application, onOpenProofModal }) => {
  ...
  return (
    <div className="bg-white border border-[#ECECE8] rounded-xl p-5 hover:shadow-sm transition-shadow">
      <div className="flex flex-col sm:flex-row sm:items-start gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            <h3 className="text-sm font-semibold text-[#0B0B0C]">{campaign?.title ?? 'Campaign'}</h3>
            {campaign?.tiers?.name && (
              <Badge variant="purple">{campaign.tiers.name}</Badge>
            )}
          </div>
          {campaign?.description && (
            <p className="text-xs text-[#8C8C88] line-clamp-2 mb-2">{campaign.description}</p>
          )}
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-[#8C8C88]">
    ...
    <div className="p-6 lg:p-8 max-w-5xl mx-auto">
      ...
      {loading ? (
        <div className="space-y-4">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-white border border-[#ECECE8] rounded-xl h-24 animate-pulse" />
          ))}
        </div>
      ) : error ? (
        <Card>
          <CardBody>
            <div className="text-center py-4">
              ...
              <button onClick={fetchApplications} className="mt-2 text-sm text-[#6E5BFF] underline">
                Try again
              </button>
            </div>
          </CardBody>
